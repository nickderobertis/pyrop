...
from typing import Callable, NoReturn, Set, Tuple, Type, TypeVar, Union
...
@pytest.mark.parametrize(
    "value,left_type,right_type", [(42, int, str), ("asdf", str, int)]
)
def test_from_union_left(
...
def test_from_optional_left() -> None:
    assert Either.from_optional(None) == Either.left(None)


def test_from_optional_right() -> None:
    assert Either.from_optional(42) == Either.right(42)
    assert Either.from_optional([]) == Either.right([])
    assert Either.from_optional(False) == Either.right(False)
...
def test_map_left(
    input: Either[A, B], f: Callable[[A], C], expected: Either[A, B]
) -> None:
...
def test_flat_map(
    input: Either[A, B], f: Callable[[B], Either[A, C]], expected: Either[A, B]
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        (
            Right(42),
            lambda x: x < 0,
            lambda x: BippyException(f"fail{x}"),
            BippyException,
            "fail42",
        )
    ],
